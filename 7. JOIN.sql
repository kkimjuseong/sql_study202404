...
-- 사원명과 소속 부서명을 조회 : 사원정보는 전부 조회.
-- 사원명과 소속 부서명을 조회
SELECT
*
FROM tb_emp E
LEFT OUTER JOIN tb_dept D  -- outer 생략 가능
ON E.dept_cd = D.dept_cd
;
...
--    D.dept_nm
FROM tb_emp E
-- LEFT는 왼쪽은 일단 다 보여줘 인데 여기서 왼쪽은 tb_emp 가 된다. 즉 사원정보는 전부!!
-- outer의 경우 매칭이 안되면 전부 null로 표시한다. 즉 tb_dept는 null!!
...
-- 오라클 조인
-- left : 조인조건(WHERE절)에서 오른쪽에 (+) 를 붙여줌  () 붙여야해~
-- right: 조인조건에서 왼쪽에 (+) 를 붙여줌
-- full: 지원하지 않는다.
-- 만약 + 가 붙으면 표준조인에서 on절에 위치하는 조건.
SELECT
    E.emp_nm,
    D.dept_nm
...
-- 부서에 소속된 사원이 41명
-- 사원이 존재하는 부서 14개
-- INNER JOIN의 결과는 41건이 나옴
-- LEFT JOIN : INNER 41건 + 부서가 없는 사원 5명 = 46건
-- RIGHT JOIN : INNER JOIN 41건 + 사원이 없는 부서 2건 = 43건
-- FULL JOIN : INNER JOIN 41건 + 부서없는사원 5명 + 사원없는부서 2건 = 48건
