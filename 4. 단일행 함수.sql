...
INSERT INTO varchar_compare VALUES ('103', 'SQLD', 'SQLA');
COMMIT;
...
SELECT 
    ...
    LTRIM('HELLOhello', 'HE')       -- 왼쪽에서 발견된 he를 제거함. 단 EL처럼 중간에 있으면 제거 불가능
FROM dual;
...
SELECT 
    ...
    TRUNC(38.678, 2) AS trunc,  -- 자리수 이하를 절삭 버려!!
    ABS(-20) AS abs,   -- 절대값
...
SELECT 
     ...
     -- NVL 1건이라도 조회가 되야 변환이 가능하다 
     -- 조회가 되지 않으면 null값이 아니라 없는 값이기 때문이다.
     ADDR 
FROM tb_emp
WHERE emp_nm = '이정직'
;
...
SELECT 
    -- emp_nm   -- 이승엽이 존재하지 않기때문에 0이 출력
    -- MAX(emp_nm)  -- 없으면 null이 조회가 됨
    -- NVL(emp_nm, '존재안함') AS emp_nm
...
SELECT 
    emp_nm,
    NVL2(direct_manager_emp_no, '일반사원', '회장님') AS 직위  -- null이 아니면 일반사원 null이면 회장님
FROM tb_emp
;
