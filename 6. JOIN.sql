...
-- INNER JOIN
-- 두 테이블 간에 연결컬럼을 이용하여 관계가 있는 데이터를 매칭하여 JOIN
SELECT
    *
FROM test_a, test_b
...
SELECT
    ...
    D.dept_nm
FROM tb_emp E 
INNER JOIN tb_dept D  -- INNER는 기본값이라 생략이 가능함.
...
-- INNER JOIN은 두 테이블 간에 연관데이터가 있는 경우에만 가능하다.
-- Ex) 어떤 쇼핑몰회원이 한건도 주문하지 않았을경우 INNER JOIN에서는 해당 회원이 조회가 되지 않음.
