...
-- where 절은 무조건 전개가 다 끝난 다음에 필터링이 됨.
...
-- # 서브 쿼리 : SQL안에 SQL이 포함된 구문
-- ## 단일행 서브쿼리 : 조회 결과가 1건이하
...
-- 행이 2개인경우는 안돔~ '이관심'이 속한 부서코드가 2개야 ㅇ ㅣ친구야 안된다고
...
-- EXISTS문 : 메인쿼리의 비교조건이 서브쿼리의 결과 중  (exists 존재하다)
--           만족하는 값이 하나라도 존재하면 참
...
WHERE EXISTS (
    SELECT
        dept_cd -- emp_nm으로 매칭해도 똑같은 값 나옴 * 써도되고 아무거나 써도 지장없다
    FROM tb_emp E
    WHERE addr LIKE '%강남%'
