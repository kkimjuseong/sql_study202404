...
-- 집계함수는 집계를 진행 할 때 NULL을 집계함 COUNT(*)의 경우는 NULL도 포함
...
-- GROUP BY에 사용한 컬럼만 SELECT에서 직접조회 가능
SELECT
    emp_no,
    COUNT (*)
FROM tb_emp
...
-- 사원별로 2019년에 급여 평균 액수와, 최대, 최소 지급액 조회
...
GROUP BY emp_no
HAVING AVG(pay_amt) >= 4500000
ORDER BY emp_no  -- SUM(pay_amt) ,  연봉 이 가능함.
;
